 ...
 * @since      1.0.0
 */
class EasyNote_Popular_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-easynote-popular widget_posts_thumbnail',
			'description' => __( 'Display popular posts by comments with thumbnails.', 'easynote' )
		);

		// Create the widget.
		parent::__construct(
			'easynote-popular',                                   // $this->id_base
			__( '&raquo; Popular Posts Thumbnails', 'easynote' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $popular = get_transient( 'easynote_popular_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'easynote_popular_widget_' . $this->id, $popular );
	...
	function flush_widget_transient() {
		delete_transient( 'easynote_popular_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Popular Posts', 'easynote' ),
			'limit'     => 5,
			'show_date' => false
		);
